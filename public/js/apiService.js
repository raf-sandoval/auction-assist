...
export async function fetchAuctionDataFromProxy(make, model, yearFrom, yearTo, maxPages = 20) {
  let allCars = [];
  let page = 1;
  ...
  while (page <= totalPagesFromAPI && !lastPageReached && page <= maxPages) {
    ...
    try {
      ...
      if (!resp.ok) {
        console.error(`API Proxy error for page ${page}: ${resp.status} ${resp.statusText}`);
        ...
        if (page === 1 && (resp.status === 404 || resp.status === 500)) {
            ...
            throw new Error(`Critical API proxy error on first page: ${resp.status}`);
        }
      ...
      console.error("Error fetching data via proxy for page " + page + ":", err);
